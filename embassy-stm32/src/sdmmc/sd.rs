...
impl<'a, 'b> StorageDevice<'a, 'b, Card> {
    ...
    fn make_ext_register_argument(fno: u8, page: u8, offset: u16, buffer: &DataBlock) -> u32 {
        // Argument Structure:
        // [31] = 0 (MIO Memory)
    ...
    async fn read_ext_register(&mut self, fno: u8, page: u8, offset: u16, buffer: &mut DataBlock) -> Result<(), Error> {
        let argument = Self::make_ext_register_argument(fno, page, offset, buffer);
        let mode = DatapathMode::Block(block_size(size_of_val(buffer)));
    ...
    async fn read_ext_registers(&mut self) -> Result<(), Error> {
        ...
        self.read_ext_register(0, 0, 0, &mut data_block).await?;
    ...
    async fn parse_ext_reg(&mut self, register_set: &[u8]) -> Result<(usize, ExtensionRegister), Error> {
        ...
        let register = if sfc & 0x1 != 0 {
            let mut data_block = DataBlock([0u32; 128]);
            self.read_ext_register(fno, page, offset, &mut data_block).await?;
    ...
    /// the card to indicate that shutdown of card power is imminent. After waiting for 1s, it is possible
    /// to turn off power to the card without risk of data loss.
    pub async fn power_off_notify(&mut self) -> Result<(), Error> {
        if let Some(PowerExtensionRegister {
            fno,
            page,
            offset,
            supports_power_off_notification,
            ...
            && *supports_power_off_notification
        {
            ...
            buffer[0] = 0x1;
            self.write_ext_reg(*fno, *page, *offset, &buffer).await
        } else {
...
impl<'a, 'b> StorageDevice<'a, 'b, Emmc> {
    ...
    /// Send a power off notification to the card. After waiting the duration specified in the ExtCSD
    /// register, the card is ready for shutdown.
    pub async fn power_off_notify(&mut self) -> Result<(), Error> {
        ...
        self.poll_ready_for_data(Some(timeout.as_ticks() as u32))
    }
}
